...
class OCRTask(BaseTask):
    ...
    def process(self, input_path, save_dir=None, visualize=False):
        ...
        for directori in dir_list:
            ...
            if(sw_separarPlanes):    
                print('Separa planes '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                for prefpath in file_list:  # Per cada PDF
                    self.separa_i_orienta(prefpath)  
                if(sw_nomesSeparar):
                      sys.exit()  
                file_list = self.prepare_input_files(directori)  
            self.numPlanes=0     
            ...
            for fpath in file_list:  # Per cada PDF
                if fpath.endswith(".pdf") or fpath.endswith(".PDF"):
                    ...
                    nom=''
                    pdf_res = []
                    print('Comença document '+basename+' '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                    self.guardar_logs('Processat '+plataforma+' ('+basename +')',0,0,nom)
                    print('--------------------------------------------------')
                    print(basename)
                    print('--------------------------------------------------')  
                    for page, img in enumerate(images):  #per cada imatge
                        self.reglog= datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]+' '  
                        self.reglog+=basename+' '+str(page+1)+ '\r\n\t'
                        self.numPlanes=self.numPlanes+1
                        #self.guardar_logs('Processant '+basename,0,1,basename +'_'+ f"plana_{page+1}.pdf")
                        ...
                        if save_dir:
                            ...
                            if(plant==''):  
                                print('Detectant plantilla '+basename+' '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                            ...
                            if(plant is not None and sw_error==False):
                                if (camps)==[]:
                                    print('Detectant camps '+basename+' '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                                ...
                                if (x>3)  : 
                                        ...
                                        try:    
                                            ...
                                            data_obj = datetime.strptime(textdata,'%d-%m-%y')
                                                ...
                                                try:
                                                 ...
                                                 self.guardar_logs('La data no te format correcte',1,2, basename +'_'+ f"plana_{page+1}.pdf")
                                                 sw_error=True   
                                    except:
                                        sw_error=True
                                    ...
                                    self.guardar_logs('No s\'han trobat tots els camps',1,2, basename +'_'+ f"plana_{page+1}.pdf")
                                    sw_error=True
                            else: 
                                print ('No s\'ha trobat la plantilla')
                                self.guardar_logs('No s\'ha trobat la plantilla',1,2, basename +'_'+ f"plana_{page+1}.pdf")
                                sw_error=True
                        ...
                        #print(nom)     
                        print('Guarda document '+nom+' '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]) 
                        self.guardarPDF(nom,img,carpeta)
                        self.guardar_logs('Processat '+plataforma+' ('+basename +'_'+ f"plana_{page+1}.pdf"+')',0,1,nom)
                        self.reglog+=nom
                        ...
                        self.reglog+='\r\n'        
                        # print(self.reglog)  
                        print('Guarda log Plataforma '+basename+' '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])          
                        self.save_log()   
                        if visualize:
                    ...
                    print('Elimina document '+basename+' '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                    os.remove(fpath)  
                          
            if(self.numPlanes>0): 
                t_estadistica=(str(self.numPlanes)+'#'+str(self.documents)+'#'+str(self.correctes)+'#'+str(self.revisions))
                self.guardar_logs('Processant '+plataforma+' ('+self.nomDocumentProcessat+')',0,0,self.numPlanes,t_estadistica)
    ...
    def carregaDadesNavision(self):
        print('Carregant Clients del CSV '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
        with open('assets/inputs/CliGrupNegoci.csv', encoding='latin1') as f:
          cont=0
          for linea in f:
            cont=cont+1
            self.dadesGrupNegoci.append(linea.strip().split(';')) 
        print('Clients CSV carregats '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
        

    def selectCodiClientNav(self, plataforma, prove, codi):
        print('buscar client Navision '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
            ...
            if(codi==i[1]):
                if(prove==i[0] or prove=='*'):
                    if(prove=='*' and ('SERHS'==i[0] or'VP'==i[0])):
                        if(plataforma==i[2]):
                            print ('Trobat client SERHS Navision '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                            return i
                    else:       
                        if(plataforma==i[2]):
                            print('Trobat client CCEP/DDI Navision '+datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                            return i
        return None
    ...
    def guardar_logs(self,descripcio,gravetat,nivell,document,estadistica=None):       
        wdata = datetime.now()
        ...
        buffer="'PDF-EXTRACT','10.252.252.32','pdf_extract','"+str(nivell)+"','"+str(descripcio)+"','"+str(estadistica)+"','"+str(gravetat)+"','"+str(wdata)+"',INTNAVANT,'"+str(document)+"','','','','','','','','','',''\n"  #+'PDF-EXTRACT,10.252.252.32,pdf_extract,'+nivell+','+descripcio+','+estadistica+','+gravetat+','+wdata+','+INTNAVANT+','+document+',,,,,,,,,\n'  
        with open('outputs/LogGeneral.txt', 'a', encoding='utf-8') as logGen:
            logGen.write(buffer)
        #print(buffer)
